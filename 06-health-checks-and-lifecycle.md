...
- **successThreshold**: consecutive successes required to flip from Unhealthy → Healthy. Must be `1` for liveness and startup probes (API server rejects other values). For readiness, raising this above 1 prevents flapping (pod bouncing in/out of Service endpoints) at the cost of slower recovery.
  - **Why liveness/startup are locked to `1` but readiness isn't:** for liveness, the only question is a binary "is the process alive right now" — one good response is sufficient proof of that; there's no reason to demand repeated confirmation before considering it recovered. Readiness is different: you can deliberately set `successThreshold` above 1 there specifically to prevent flapping — e.g. requiring 2-3 consecutive successful checks before re-adding a pod back into the Service's endpoints, so a borderline-flaky pod (succeeds once, fails again) doesn't bounce in and out of receiving traffic repeatedly.
